...
public class FileExample1 {
	...
	/*
	 * File 클래스 : file 객체를 생성하는 클래스. 파일(파일과 디렉터리)과 과련된 작업을 진행할 수 있음.
	 */

	static String env_path = "C:\\Program Files\\Python313\\Scripts\\;C:\\Program Files\\Python313\\;C:\\Program Files\\Java\\jdk-21\\bin;"
			...
			+ "C:\\Program Files (x86)\\NVIDIA Corporation\\PhysX\\Common;C:\\Program Files (x86)\\Windows Kits\\10\\Windows Performance Toolkit\\;"
			+ "C:\\Users\\Admin\\AppData\\Local\\Microsoft\\WindowsApps;C:\\Users\\Admin\\AppData\\Local\\Programs\\Microsoft VS Code\\bin";

	public static void main(String[] args) {
		// file 객체 생성

		File f = new File("D:/Temp/a.txt"); // 파일 정보를 담은 객체
		...
		// - D:\Temp\a.txt
		// File 작업에서 디렉터리 구분값을 "\\" 또는 "/"를 사용

		// 상대 경로 : 현재 작업 위치로부터 경로를 표시하는 경우
		// ./ : 현재 디렉터리, ../ : 상위 디렉터리(바로 위)
		// ../test/test.txt -> 부모 디렉터리에서 test디렉터리에 있는 test.txt파일
		System.out.println("isAbsolute? : " + f.isAbsolute());
		System.out.println("isAbsolute? : " + dir.isAbsolute());

		// 4. canExcute, canRead, canWrite : 실행권한, 읽기권한, 쓰기권한
		...
		// 6. 부모 폴더를 단순문자열로 반환 : getParent()
		String parentDir = f.getParent();
		System.out.println(parentDir);

		// 7. 부모 폴더를 file객체로 반환 : getParentFile() -> file 오브젝트임
		File f_parent = f.getParentFile();
		System.out.println(f_parent);

		// 8. File 상수값 : 구분값 기호
		System.out.println(File.separator); // \ 디렉터리 구분값 String타입
		System.out.println(File.separatorChar); // \ char타입
		System.out.println(File.pathSeparator); // ;(세미콜론)으로 경로 구분값 String타입
		System.out.println(File.pathSeparatorChar); // ; char타입
		...
		if (!test.exists()) {
//			test.mkdir();	// 경로에 부모 디렉터리중 하나라도 없으면 생성을 못한다, 반환값 boolean
			test.mkdirs();
		}
		...
		try {
			...
			if (!testDir.exists()) { // ! -> 논리 부정 연산자임
				if (testDir.mkdir()) {
					...
					throw new FileNotFoundException("파일 경로를 못찾았습니다.");
				}
			}
			test.createNewFile(); // 반환 타입이 boolean
		} catch (IOException e) {
			e.printStackTrace();
