 ...
 * 	 - 인터페이스에 포함된 메서드들은 모두 abstract mathod(기본)이다.
 * 	 - abstract class와 다르게 인스턴스 변수와 인스턴스 메서드를 만들 수 없음.
 * 	 - static 변수와 static 메서드는 사용이 가능하다. (자동으로 fanal이 된다)
 * 	 - 클래스와 다르게 구현체가 여러개 일 수 있어요.
 */

public interface RemoteControl {
	
	// 상수 필드(멤버 변수) : 인터페이스 내에 멤버변수(필드) 설정을 하면 이것이 상속 값이 됩니다.
	// public static final 이 생략됨...
	int MAX_VOLUME = 10;
	int MIN_VOLUME = 0;
	
	// 추상 메서드 -> 구현측에서 반드시 재정의해야 함. (implemetation)
	...
	// default 메서드 -> {}를 사용하여 코드 실행부분이 존재함.
	...
	// 실행부 작성시에 상수 필드(멤버변수)를 제외한 추상메서드, 디폴트 메서드, private메서드 등을 호출 할 수 없다.
	
	static void chengeBattery() {
