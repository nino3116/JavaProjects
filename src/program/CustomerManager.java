...
public class CustomerManager {
	...
	static final int MAX = 100; // 맥스가 100이면 인덱스는 99까지만 확인가능
	
	// 고객정보를 저장할 변수를 배열로 선언(이름, 성별, 이메일, 탄생년도)
	...
	// 때문에 현재 데이터가 몇 개 저장되어 있는지 알 수 있어야 합니다.
	static int count = 0;
	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			switch(menu.charAt(0)) { 
				case 'i':
					System.out.println("고객 정보를 입력을시작합니다.");
					if(count >= MAX) {
						System.out.println("더 이상 저장할 수 없어요.");
					}else {
						// 고객 정보를 저장하는 기능... insertCustomarData() 메서드를 구현...
						insertCustomarData(index);
					}
					break;
				case'p':
					...
					if(index <= 0) {
						...
						// index를 이용하여 데이터를 출력..
						index--; // index = index - 1; , or index -=1; 과 같음
				...
				case 'u':
					...
					// update를 선택하면 현재 인덱스의 자료를 수정하여는 메서드를 구현
	...
	public static void insertCustomarData(int index) {
		System.out.print("이름 : ");
   ...
   public static void deleteCustomarData(int index) {
