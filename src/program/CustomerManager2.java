...
public class CustomerManager2 {
	...
	// 멤버 변수 선언
	static final int MAX = 100; // 상수값처리
	
	// 고객 정보를 저장할 변수를 배열로 선언(이름, 성별, 이메일, 출생년도)
	// Customer 클래스를 생성해서 객체 배열로 선언
	static Customer[] cusList = new Customer[MAX]; // Customer 배열 // 고객정보 배열 할당
	
	// 기존에 사용했던 index는 사용 안해도 됨... 객체 내에 고유 정보를 통한 검색을 이용할 예정.
	
	// 대신에 count사용은 함.. 카운트는 저장한 데이터의 크기.
	static int count; // 참조 타입이기 때문에 기본 값으로 자동으로 초기값으로 설정된다 
	...
	public static void 	insertCustomerData(){
		 ...
		 * c1.setGender(gender);
		 * c1.setEmail(email);
		 * c1.setBirthYear(birthYear); // 도 사용가능하다~
		 * 
		 */
		cusList[count] = c1;
		count++;
	...
	public static void updateCustomerData(Customer cus) {
		...
		String name = scan.next();
		cus.setName(name);
//		cus.setName(scan.next()); 이렇게해도 된다.
	...
	public static void deleteCustomerData(Customer cus) {
		for (int i = 0; i < count; i++) {
			...
			count --; // count를 줄이면, 마지막 위치가 변경이 일어납니다.
		}	
	}	// deleteCustomerData 끝
