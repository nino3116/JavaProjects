...
public class CheckID {
	 ...
	 /*
	    ...
	    *  
	    *  예외가 발생한 경우...
	    *  공백이 존재합니다. 혹은 중복된 아이디 입니다. 혹은 유요하지 않은 문자가 있습니다.
	    *  글자수 제한은 20글자 입니다.
	    */
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			System.out.print("입력 >> ");
			int cmd = 0;
			try {	// 입력값으로 숫자가 아닌 문자열을 입력받을 경우에 예외처리하기 위해
				cmd = scan.nextInt();
			} catch (InputMismatchException e) {
				System.out.println("정수 이외 입력하지 마세요!!");
				scan.nextLine();    // 버퍼 정리
				e.printStackTrace(); // 어디서 무엇이 잘못됐는지 알려주는 명령어.
			}
		
			// switch - case 문을 사용하여 기능
			switch (cmd) {
			case 1: {	// 아이디 등록
				scan.nextLine(); // 버퍼 정리를 위해서 사용함!
				...
				// 메서드만들기 :  addNewMember(new_id), 반환타입 boolean
	...
	static boolean addNewMember(String id) {	// 아이디 등록 처리하는 메서드
		/* 1) 아이디 사이에 공백이 있는지, 
	     * 2) 중복된 아이디 인지 확인,
	     * 3) 영어, 숫자,_만 사용가능(다른 문자 사용 불가능), 
	     * 4) 글자수 제한 (20자) 
		...
		id = id.trim().toLowerCase();	// 양끝 공백제거, 소문자로 변환
	...
	static boolean chkIdDuple(String id) {
		...
		// chkValidChar(id) 영어, 숫자, _만 가능.. true(유효한문자용), false(유요한문자사용X)
