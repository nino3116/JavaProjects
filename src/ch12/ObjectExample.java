 ...
 * 	
 * 	- equals() : 객체의 번지를 비교하여 boolean 값으로 비교 결과를 리턴 (재정의를 통해서 객체 비교시 사용)
 * 	- hashCode() : 객체를 식별하는 정수, 객체의 메모리 번지(주소)를 이용하여 해키코드 생성.
 * 				   때문에 생성되는 객체마다 다른 정수값을 가지게 됨. 이 기능을 이용하여 동등 객체 비교시 사용
 * 	- toString() : 객체를 문자열로 표현한 것, -> 클래스명@16진수해시코드
 * 				   보통은 각 클래스에 맞게 재정의해서 사용하는 경우가 많음
 * 	
 * 	** 레코드 선언
 * 		데이터 전달을위한 DTO 작성시 반복적으로 사용되는 코드를 줄이기 위해서 도입됨
 * 
 */	 
...
public class ObjectExample {
	...
	public static void main(String[] args) {
		...
		System.out.println("p1.heshCode() : " + p1.hashCode());
		System.out.println("p2.heshCode() : " + p2.hashCode());
		System.out.println("p3.heshCode() : " + p3.hashCode());
...
// record 키워드는 해당 클래스가 DTO(데이터 전달을 위한 객체)를위한 클래스임을 나타냄
