...
public class OperaterEx1 {
	...
	public static void main(String[] args) {
		...
		System.out.println(x / y);		// 0 , 0.5가 아닌이유. 정수라서
		...
		byte over = (byte)(127 + 1); // 128을 이용한 오버플로우
		...
		// Double.isInfinit()와 Double.isNaN() 메서드(함수)를 통해서 무한대 혹은 NaN인지 확인이 가능함
		
		System.out.println( Double.isInfinite (5/0.0) );	// 무한대인지 여부확인
		System.out.println( Double.isNaN (5%0.0) );		// NaN인지 여부 확인
		
		// 비교 연산자
		// (동등비교) : ==(같다), !=(다르다)
		// (크기비교) : <, <=, >, >=,  왼쪽(주체)오른쪽(계산)
		// 비교 결과는 boolean 타입으로 반환 -> ture/ false
		...
		// 문자열 비교시.... equals() 도는 !equals()를 ==, != 연산자 대신 사용 
		System.out.println("문자열 비교 -> test 와 tset의 비교");
		System.out.println("test" .equals("test"));  	// 같은지 여부 확인
		System.out.println(!"test" .equals("test")); 	// 같지 않은지 여부 확인
		
		// 논리 연산자
		// 논리곱(&&)-and, 논리합(||)-or, 배타적 논리합(^), 논리부정(!)
		// 비교 연산자와 같이 흐름제어를 위한 if, 반복문 for, while에서 주로 사용함.
		...
		System.out.println("결과 : " + (x > y && ++x == 10)); // 숏컷연산 ++x == 10 데드코드라 연산 안함
		...
		// 비트 이동 연산자
		// 비트 좌측 또는 우측으로 밀어서 이동시키는 연산을 수행
		// <<, >>, >>>
		//<<
