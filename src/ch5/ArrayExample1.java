...
public class ArrayExample1 {
	...
	public static void main(String[] args) {
		...
		// 3. 타입명[] 변수명 = new 타입명[]{ 데이터들 }; // 데이터는 ","로 구분
		
		int[] numbers = new int[10];	// 정수형 변수 10개를 가진 배열..
		// int는 0으로 초기화 됨... 정수타입은 전부 0으로 초기화 된다. 
		...
		// 배열의 길이 알아오기
		// 배열.length : 배열의 길이 값이 저장되어 있다.
		System.out.println("win[]의 길이 : "+win.length);
		System.out.println("welcome[]의 길이 : "+welcome.length);
		...
		String[] byeMessage2 = new String[] {"good bye","bey bey","see you"};
		
		// 얖은 복사(swallow copy) -> 참조 객체 주소를 복사...
//		byeMessage = byeMessage2;  // 1000번지 주소를 참조하다가 Message2를 대입하면 2000번지주소를 참조하게됨
//		
//		System.out.println(byeMessage);  // 출력해도 같은 값이 나옴
